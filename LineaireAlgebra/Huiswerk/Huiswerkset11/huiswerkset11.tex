...
\section*{Opgave 9.4.2(2)}
Kies $v_1 = (1,2)$ voor $L$ en $v_2 = (-2, 1)$ voor $L^\bot$.
...
\]
\section*{Opgave 9.4.2(3)}
Basiswisselmatrix $P$ van $C$ naar $B$:
...
\subsection*{Basisstap $n = 1$}
$A = (a_{11})$, dus $\det(A) = a_{11}$

\subsection*{Inductiestap}
Stel het geldt voor $(n-1) \times (n-1)$ matrices.

Cofactor-expansie naar eerste kolom:
\[\det(A) = a_{11} \cdot \det(A_{11}) + 0 + \ldots + 0\]

Submatrix $A_{11}$ is ook bovendriehoek met diagonaal $(a_{22}, \ldots, a_{nn})$.
