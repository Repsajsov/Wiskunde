...
Herschrijf:
\begin{align*}
\left|x^4\cdot\cos(\f{1}{x})-f(0)\right| &= \left|x^4\cdot\cos(\f{1}{x})\right|  \\
&\leq |x^4| \quad (\tx{Omdat $-1 \leq \cos\left(\f{1}{x}\right) \leq 1$})\\
&= |x|^4 < \delta^4
\end{align*}
Kies $\delta = \sqrt[4]{\epsilon}$, dan:
\[\left|x^4\cdot\cos(\f{1}{x})-f(0)\right| < \delta^4 = (\sqrt[4]{\epsilon })^4 = \epsilon\]
Hieruit volgt dus dat voor alle $\epsilon$ een $\delta$ kunnen vinden en daarmee is de functie continue op $c=0$.
\end{proof}
\section*{Opgave 3}
Kies epsilon $\epsilon = \f{1}{2}$, voor elke $c$ die we dan hebben in $(c - \delta, \ c + \delta)$ geldt dat er zowel rationale als irrationale getallen zitten.
...
\section*{Opgave 4} 
Per term bekijken we of het continue is en vervolgens moet de som van alle termen dan ook continue zijn.
\begin{itemize}
    \item 
    Merk op dat $|x|$ continue is, omdat $|x^3| = |x|^3$ is $|x^3|$ dus ook continue volgens de samenstelling van continue functies.
\item
